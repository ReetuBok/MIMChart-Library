...
-(void)drawMultiWallGraph
{
    ...
    for (int k=0; k<outerLoop; k++) 
    {
        ...
        {   
            ...
            if(l==0)
                [myLinePath moveToPoint:CGPointMake(valueX*_scalingX , valueY*_scalingY)];
            else
                [myLinePath addLineToPoint:CGPointMake(valueX*_scalingX , valueY*_scalingY)];
...
-(void)initAndWarnings
{
    ...
    if([delegate respondsToSelector:@selector(AnchorStylesForWallChart:)])
    {
        anchorArray=[delegate AnchorStylesForWallChart:self];    
...
- (void)drawRect:(CGRect)rect
{
    ...
    if(_gridWidth > self.frame.size.width)
    {
        ...
        {
            colorLineChart=[delegate ColorsForWallChart:self];
            ...
            {
                graph.colorWallChart=[[NSArray alloc]initWithArray:colorLineChart];
            }
        ...
        if(pickDefaultColorForLineChart)
        {
            ...
            graph.colorWallChart=[[NSArray alloc]initWithArray:colorA];
        }
    ...
    BOOL pickDefaultColorForLineChart=FALSE;
    NSArray *colorLineChartArray;
    
    
    if([delegate respondsToSelector:@selector(ColorsForWallChart:)])
    {
        colorLineChartArray=[delegate ColorsForWallChart:self];
    ...
    for (int i=0; i<[myPathArray count]; i++) 
    {
        ...
        if(pickDefaultColorForLineChart)
        {
            ...
            _color=[[UIColor alloc]initWithRed:red green:green blue:blue alpha:0.3]; 
            [_color setFill];
        ...
        else
        {
            ...
            _color=[[UIColor alloc]initWithRed:colorLineChart.red green:colorLineChart.green blue:colorLineChart.blue alpha:0.3]; 
        ...
        [self drawWallEdge:ctx AtIndex:i WithColors:r :g :b];        
    }
    ...
    CGContextSetAllowsAntialiasing(ctx, NO);
    CGContextSetShouldAntialias(ctx, NO);
    //[self drawThePattern:ctx]; //slanting lines on the wall.
...
-(void)drawWallEdge:(CGContextRef)ctx AtIndex:(int)index WithColors:(float)red :(float)green :(float)blue
{
