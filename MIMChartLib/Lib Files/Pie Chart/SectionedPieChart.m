...
@implementation SectionedPieChart
...
//    
    ...
    //divider
    //Add Divider Color WITH GRADIENT
    
    if(!dividerInfoProvided)
    {
        
        float dWidth=self.frame.size.width;
        
        
        CGFloat BGLocations3[2] = { 0.0, 1.0 };
        CGFloat BgComponents3[12] = {0.023,.239,.403 , 1.0,  // Start color
            .666,.827,0.8 , .909}; // End color
        CGGradientRef gradient = CGGradientCreateWithColorComponents(BgRGBColorspace, BgComponents3, BGLocations3, 2);
        
        CGContextSaveGState(context);
        CGContextAddEllipseInRect(context, CGRectMake(10, pieChart.centerY_-1, dWidth-20, 2));
        CGContextClip(context); 
        
        CGPoint startPoint  = CGPointMake(pieChart.centerX_, pieChart.centerY_); 
        CGPoint endPoint =CGPointMake(pieChart.centerX_, pieChart.centerY_);
        CGContextDrawRadialGradient(context, gradient, startPoint, pieChart.centerX_/2,endPoint , pieChart.centerX_, kCGGradientDrawsBeforeStartLocation);
        CGContextRestoreGState(context);
