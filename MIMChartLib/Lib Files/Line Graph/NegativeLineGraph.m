...
@implementation NegativeLineGraph
...
-(void)drawNegativeLineGraph
{
    ...
    for (int k=0; k<outerLoop; k++) 
    {
        ...
        {   
            ...
            valueY=((fabs(minimumOnY)+1) * _tileHeight)+(valueY*_scalingY);
            
            if(l==0)
                [myPath moveToPoint:CGPointMake(valueX*_scalingX , valueY)];
            else
                [myPath addLineToPoint:CGPointMake(valueX*_scalingX , valueY)];
...
-(void)_findScaleForYTile
{
    ...
    minimumOnY=fabs(minOfY/pixelsPerTile);
    minimumOnY=ceilf(minimumOnY);
    
    //If it is negative, then start it from minimumOnY.
    if(minOfY <0)minimumOnY=-minimumOnY;
...
//        myPath.miterLimit=5.0;
        ...
        {   
            ...
            float mY=((fabs(minimumOnY)+1) * _tileHeight)+(valueY*_scalingY);
            mY=_gridHeight-mY;
...
-(void)_displayYAxisWithColorRed:(float)red Blue:(float)blue Green:(float)green Alpha:(float)alpha
{
    if([delegate respondsToSelector:@selector(displayTitlesOnYAxis:)])
    {
        ...
        if(displayY)
        {
            ...
            [_yBand setScaleForYTile:pixelsPerTile withNumOfLines:numOfHLines  withMin:minimumOnY*pixelsPerTile];
